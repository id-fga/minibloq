...
bool Bubble::deploy()
{
    ...
    if (build())
    {
        wxString puerto = getBootPortName();
        ...
        wxString tmp = wxString(getHardwareManager()->getCurrentBoardProperties()->getPath() + wxString("/main.board"));
        printf("\tdeploy()\n");
        printf("\t\tLevanto los comandos de deploy de ->%s\n",tmp.mb_str().data());
...
////////////////////////
        ...
        wxString tmp = wxString(getHardwareManager()->getCurrentBoardProperties()->getPath() + wxString("/main.board"));
        printf("\t\tLevanto los comandos de build de ->%s\n",tmp.mb_str().data());
        //cmd = bubbleXML.parseCmd(cmd);
...
bool Bubble::generateCodeAndSaveToFile()
{
	...
	//Elimino los archivos para crearlos con el nuevo contenido
	if(wxFileExists(mbqGlogalsHeaderName)) {
        	wxRemoveFile(mbqGlogalsHeaderName);
	}

	printf("Aca quiere eliminar %s\n", mbqGlogalsHeaderName.mb_str().data());
	...
	if(wxFileExists(initBoardFileName)) {
	...
	if(wxFileExists(mainOutputName)) {
...
//        dialog0.ShowModal();
    ...
    //Si el parent no es null, no se puede maximizar
    //actionPicker = new BubbleActionPicker(  pickerParent,
    actionPicker = new BubbleActionPicker(  NULL,
                                            this,
                                            wxNewId(),
                                            getActionDataType(),
                                            _("Actions"),
