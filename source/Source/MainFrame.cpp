                                        ...
                                        componentAlreadySaved(false)
{
    ...
    bubble.setThemePath(bubble.getAppPath() + "/../../GUI/Images/Theme0/");
...
//    dialog0.ShowModal(); //##Debug
    ...
    wxString homeDirectory = wxGetUserHome();
    wxFileName _tempPath(homeDirectory + "/miniBloq/");
...
{
    //TODO: Hacer un archivo de configuracion de usuario y una system-wide
    //TODO: Si no existe, crear uno por default
    wxString homeDirectory = wxGetUserHome();
    wxString configDir = homeDirectory + "/.config/miniBloq/miniBloq.xml";
    return configDir;
    //return  wxStandardPaths::Get().GetExecutablePath().BeforeLast(wxFileName::GetPathSeparator()) + wxString("/config/miniBloq.xml");
}
...
void MainFrame::readConfig()
{
    ...
    printf("El archivo de configuracion es %s\n",fileName.mb_str().data());
...
void MainFrame::openExample()
{
    ...
    wxString homeDirectory = wxGetUserHome();
    wxString folderPath = homeDirectory + "/miniBloq_ejemplos";
...
void MainFrame::toggleWindow(const wxString& name, wxMenuItem *menuItem)
{
	...
	//TODO: Superhardcode, prefiero una aplicacion por fuera que muestre la salida del serial.
	if (name == wxString("Terminal")) { //##Un-hardcode.
		...
		if (terminal)
		{
			 ...
			 */
		printf("Click en el menu Terminal\n");

			//printf("El puerto es %s\n", bubble.getHardwareManager()->getPortSelection().mb_str().data());


                	if(bubble.getHardwareManager()->getCurrentBoardProperties()) {

				wxString port = bubble.getBootPortName();
			}
		}

	} else {

		if (auiManager.GetPane(name).IsShown()) //Ya se esta mostrando
		{
